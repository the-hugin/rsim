...
triggers:
  - "ревью backend-кода"
  - ".py под api/"
output: "backend code review report: quality, security, maintainability"
calls: []
---
...
- **"The ORM protects against SQL injection"** — only if used correctly; raw queries or `.execute()` with string formatting are still vulnerable

## Quality Thresholds

Before delivering the review, verify:
- [ ] Every finding cites a specific `File:Line` reference
- [ ] Every Critical finding includes a concrete fix example (not just a description)
- [ ] Security findings include an exploitation scenario, not just "this could be vulnerable"
- [ ] No vague language ("probably", "might", "could potentially be an issue")
