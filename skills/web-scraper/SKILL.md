---
name: web-scraper
description: Scrape web pages, search the internet, and extract structured content using Python. Includes anti-bot/Cloudflare bypass via Scrapling. Use when the user wants to fetch a webpage, search for information online, extract links, crawl JS-rendered pages, or bypass bot protection.
compatibility: |
  Requires Python 3. Lightweight mode needs requests, beautifulsoup4, readability-lxml, html2text.
  Dynamic mode needs crawl4ai. Search needs duckduckgo-search.
  Anti-bot mode needs scrapling[all]>=0.4.2 (Python 3.10+).
group: domain
triggers:
  - "скрапинг"
  - "парсинг сайта"
  ...
  - "cloudflare bypass"
  - "stealth скрапинг"
output: "extracted web content: text, links, structured data"
calls: []
---
...
### 5. Anti-bot / Cloudflare protected sites

Use when `crawl_dynamic.py` returns 403, CAPTCHA, or Cloudflare challenge page.
...
```bash
# Browser automation (JS-heavy sites, not Cloudflare)
...
# Save output
python scripts/scrapling_fetch.py "URL" --stealth --save output.txt
```

Options:
- `--stealth` — Enable Cloudflare bypass (slower, more effective)
- `--selector CSS` — Extract only elements matching the CSS selector
- `--save PATH` — Save output to file
- `--max-length N` — Truncate output to N characters (default: 10000)
...
2. If `fetch_page.py` returns empty/garbled content → try **`crawl_dynamic.py`** (the page needs JavaScript).
3. If `crawl_dynamic.py` returns 403 / Cloudflare block → try **`scrapling_fetch.py --stealth`** (anti-bot bypass).
4. Need to find URLs first? → Use **`search_web.py`** to discover relevant pages.
5. Need to navigate a site structure? → Use **`extract_links.py`** to map out links, then fetch individual pages.
