...
group: domain
output: "configured notification integration (bot token, webhook, channel setup)"
calls: []
triggers:
...
> Interactive wizard: use AskUserQuestion for each step. Write result to `~/.claude/.omc-config.json`.
